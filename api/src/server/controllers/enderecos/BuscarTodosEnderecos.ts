...
const buscarTodos = async (request: Request<{}, {}, {}, IConsulta>, response: Response) => {
    const contagem = await EnderecoProvider.contar(request.query.filtro);
    const resultado = await EnderecoProvider.buscarTodos(request.query.pagina || 1, request.query.limite || 7, request.query.filtro || '');
